...
- `GET {DATA_BASE_URL}/{region}/osm-index.tsv.gz` → **gzipped** TSV, one row per OSM feature the matcher was offered as a candidate for some stop: `osm:id`, `lon`, `lat`, `flavour`, `modes`, `gtfs_seen_by`, `nearest_m`, `gtfs_matched`, `gtfs_anchored`, `gtfs_ids`, `name`. Read by `src/services/osmIndex.ts`, **only while Preview is on** and once per region — it is the largest file the report publishes (11.6 MB of text, 3.3 MB gzipped, for swiss-opendata) — and unwrapped with `DecompressionStream`, since the server compresses only `text/html`. The UI keeps the rows nothing matched, and the report omits features whose id match was promoted to a station. Absent from a report written before the matcher recorded its pool; a 404 is an absence, not an error.
