...
- `GET {DATA_BASE_URL}/{region}/osm-index.tsv.gz` → **gzipped** TSV, one row per OSM feature the matcher was offered as a candidate for some stop: `osm:id`, `lon`, `lat`, `flavour`, `modes`, `gtfs_seen_by`, `nearest_m`, `gtfs_matched`, `gtfs_anchored`, `gtfs_ids`, `name`. Its companion `osm-index-stops.tsv.gz` holds the subset a map draws — the stops and stations nothing matched — and is what `src/services/osmIndex.ts` reads, once per region, when the "unmatched OSM stops" dataset is ticked. The full pool (732,315 rows and 18 MB gzipped on germany-local, against 67,428 rows in the small file) is for analysis — it is the largest file the report publishes (11.6 MB of text, 3.3 MB gzipped, for swiss-opendata) — and unwrapped with `DecompressionStream`, since the server compresses only `text/html`. The UI keeps the rows nothing matched, and the report omits features whose id match was promoted to a station. Absent from a report written before the matcher recorded its pool; a 404 is an absence, not an error.
