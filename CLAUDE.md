...
- `GET {DATA_BASE_URL}/{region}/index.tsv?t=<now>` → one row per GTFS stop, parsed in `src/services/matchIndex.ts` **by header name, never by position**: `gtfs:id`, `kind` (the 3-letter category — `status_detailed` in older reports), `type` (picks the detail file), `lon`, `lat`, `byte_start`, `byte_end`, `strategies` (a **flag column**, despite the name: bits 1–64 are match strategies, `1<<7` and `1<<9` are dissolution planned and declined, and `1<<8` is retired and left unused), and optionally `anchor_lon`/`anchor_lat` (where the matcher anchored the stop — what Preview draws it at). `search_terms` is the stop name and matched OSM ids, and nothing reads it: flags live in the flag column, not in the column for the things someone types.
- `GET {DATA_BASE_URL}/{region}/pt-structure.pmtiles` → the stop-structure overlay, fetched only when the PT structure button is pressed, which is also when `pmtiles` itself is imported. A region without one 404s and the button simply draws nothing.
- `GET {DATA_BASE_URL}/{region}/osm-index.tsv.gz` → **gzipped** TSV, one row per OSM feature the matcher was offered as a candidate for some stop: `osm:id`, `lon`, `lat`, `flavour`, `modes`, `gtfs_seen_by`, `nearest_m`, `gtfs_matched`, `gtfs_anchored`, `gtfs_ids`, `name`. Its companion `osm-index-stops.tsv.gz` holds the subset a map draws — the stops and stations nothing matched — and is what `src/services/osmIndex.ts` reads, once per region, when the "unmatched OSM stops" dataset is ticked. The full pool (732,315 rows and 18 MB gzipped on germany-local, against 67,428 rows in the small file) is for the standalone viewer at `/osm-index.html` and for analysis — it is the largest file the report publishes (11.6 MB of text, 3.3 MB gzipped, for swiss-opendata) — and unwrapped with `DecompressionStream`, since the server compresses only `text/html`. The UI keeps the rows nothing matched, and the report omits features whose id match was promoted to a station. Absent from a report written before the matcher recorded its pool; a 404 is an absence, not an error.
...
- `StopsLayer` (`report.tsx`) is **one** source/symbol layer for all of a region's stops; category toggles use `map.setFilter` on `subcategory`, and Preview swaps the geometry with `setData` from one of two collections built per region and kept — the source is created once per region, never per toggle. Icons are data-driven `stop-{code}` images recolored from `/stop-var.svg` by `loadSvgWithColors`, with `icon-opacity` fading the stops the anchoring refused. It also mutates the stored overlay spec's `filter` and `data` in place so base-style switches re-apply what is on screen.
- Render-less overlay components follow one idiom: build the overlay spec, `mapLoaded.then(...)` guarded by a `subscription = { canceled, promiseFulfiled }` object, clean up in the effect teardown. See `StopsLayer`, `RegionMarkersLayer`, `MatchArrowLayer`, `RoutesMap` (which instead keeps a persistent `routes` source and `setData`s into it), `HtmlMapMarker` (`editor/map-marker.tsx`).
...
- Simplicity and less code: prefer the smallest change that works. **Avoid new dependencies** — the runtime deps are `preact`, `maplibre-gl` and `pmtiles`, and even XML encoding is hand-rolled — but they are not forbidden: a format with a real spec is a better reason than a convenience wrapper. One that earns its place should be **loaded on demand** (`await import(...)`), so a feature nobody switches on costs nobody anything — that dynamic import is what splits the chunk; a `manualChunks` entry only gives it a readable name. Still no state or router libraries.
