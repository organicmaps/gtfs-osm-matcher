...
- `GET {DATA_BASE_URL}/{region}/osm-index.tsv` → one row per OSM feature the matcher was offered as a candidate for some stop: `osm:id`, `lon`, `lat`, `flavour`, `modes`, `gtfs_seen_by`, `nearest_m`, `gtfs_matched`, `gtfs_anchored`, `gtfs_ids`, `name`. Read by `src/services/osmIndex.ts`, **only while Preview is on** and once per region — it is the largest file the report publishes (11.6 MB for swiss-opendata, against 9.3 MB for `index.tsv`), and the UI keeps only the rows nothing matched. Absent from a report written before the matcher recorded its pool; a 404 is an absence, not an error.
