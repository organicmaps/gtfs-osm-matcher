...
import { MapContext, SelectionContext, OsmMatchesOptionsContext } from "../app";
...
import { OsmMatchesSwitch } from "./switch";
 ...
 *
 * <p>One collection either way, with the same feature ids: the osm-matches view is a
 * re-projection of the report's own markers, not a second dataset over them. That is what lets the switch be
...
export function MatchReport({ reportRegion, reportData }: MatchReportProps) {
    ...
    const { osmMatchesOn, setOsmMatchesAvailable } = useContext(OsmMatchesOptionsContext);
    ...
    // The second is what the osm-matches view can actually move, and the categories selected
    // by default (the unmatched ones) have none of it — so without the count beside the
    // switch its first use looks like a broken control.
    ...
    // corrected it -- nothing reads it while the report has nothing to show on osm matches,
    // and the switch tells the panel's copy of itself to stop offering the control.
    const showingOsmMatches = osmMatchesOn && anchoredTotal > 0;

    useEffect(() => {
        setOsmMatchesAvailable(anchoredTotal > 0);
        return () => setOsmMatchesAvailable(false);
    }, [anchoredTotal, setOsmMatchesAvailable]);
    ...
    const featureCollection = collections(showingOsmMatches);
    ...
    // marker. With osm matches on a marker stands at its anchor, which is usually the
    ...
    // An unmatched OSM feature has no stop behind it, so there is no index row to load and
    // no detail body to range-fetch. The panel is given the one thing it needs to do its job:
    // the feature's id under `osmFeatures`, which is what its "OSM Features" section reads.
    // It fetches the live tags by id from there, exactly as it does for a matched stop's
    // features -- the report's own file carries no tags, and would be stale if it did.
    const handleUnmatchedOsmClick = useCallback((feature?: MapGeoJSONFeature) => {
        if (!feature) return;
        const { osmId, name, flavour, seenBy, nearestM } = feature.properties as {
            osmId: string, name?: string, flavour?: string, seenBy?: number, nearestM?: number,
        };
        // Every feature of this layer is a Point, built from a row's lon/lat above.
    ...
    useEffect(() => {
        if (map && selectionSource === 'url-hash' && selection) {
            // Where the marker is, not where the feed put the stop: with osm matches on the
            ...
            const drawnAt = showingOsmMatches && row?.anchorLon !== null && row?.anchorLat != null
    ...
    // The osm-matches view moves the stops rather than replacing them, so the category filters
    // keep working while it is on -- which is the point: a category is still the thing you are
    ...
    const osmMatchesControl = anchoredTotal > 0 && (
        <div className={'match-group'} key={'osm-matches'}>
            <div className={'match-group-header'}>
                <OsmMatchesSwitch />
    ...
    const unassignedLayer = showUnmatchedOsm && unassigned &&
        <UnassignedOsmLayer key={reportRegion} layerKey={reportRegion} data={unassigned}
            onClick={handleUnmatchedOsmClick} />;
...
function StopsLayer({ layerKey, data, selectedCodes, onClick }: StopsLayerProps) {
    ...
    // is not how this layer changes -- osm matches flips every stop's geometry, and tearing
    ...
    // New geometry — osm matches moving every stop to where the matcher anchored it — goes
...
/**
 * The OSM features the matcher was offered and nothing matched, while osm matches is on.
 *
 * <p>The osm-matches view answers "where did the matcher put this stop"; this answers the
 * other half of the same question — what was standing there that it did not use. Zürich Central has seven
 ...
 */
function UnassignedOsmLayer({ layerKey, data, onClick }: UnassignedOsmLayerProps) {
    const mapContext = useContext(MapContext);
    const map = mapContext?.map;
    const mapLoaded = mapContext?.loaded;
    const stylingControls = mapContext?.layerControls;
    ...
    // Read through a ref for the same reason the stops layer does: the effect below is keyed
    // on the map and the layer id, so a handler captured directly would be the one from the
    // render that created the layer.
    const onClickRef = useRef(onClick);
    onClickRef.current = onClick;
    ...
    useEffect(() => {
        ...
        const spec = {
            ...
            layers: [{
                ...
                'source': sourceId,
                'paint': {
                    // Readable before you have zoomed in, which is the point of the layer:
                    // it answers "is there unmatched OSM around here" from the city view, and
                    // a 2 px dot below z12 answered it only by accident. Held flat down the
                    // low zooms rather than shrinking to nothing, and grown from z12 where
                    // individual features start to matter.
                    'circle-radius': ['interpolate', ['linear'], ['zoom'],
                        4, 3.5, 8, 4.5, 12, 5.5, 15, 7.5, 17, 9] as any,
                    'circle-color': '#c23b22',
                    // The white ring is what separates a dot from the map under it; at the
                    // small sizes it has to grow with the dot or the two merge.
                    'circle-stroke-width': ['interpolate', ['linear'], ['zoom'],
                        4, 1, 12, 1.25, 17, 1.75] as any,
