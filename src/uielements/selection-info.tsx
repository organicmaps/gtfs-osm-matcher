 ...
 *
 * This is the other half of the osm-matches view: a stop drawn at its feed position either
 * anchored exactly there or was refused, and the map cannot tell those apart on its own.
 */
...
function MatchInfo({ datasetName, properties, geometry, idTags, reportRegion }: MatchInfoProps) {
    ...
    return (<div>
        ...
        {/* Where the IR writes this stop, and why it writes it at its own coordinates when it
            does. The osm-matches view moves a stop's marker onto its anchor, so a marker
            that did not move
